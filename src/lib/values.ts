...
type Underscored<T extends string> = { readonly [P in T as Underscore<P>]: Underscore<P> };

export type Brand<Type, Name extends string> = Type & Underscored<Name>;

export type UnArray<T> = T extends Array<infer A> | ReadonlyArray<infer A> ? A : never;
...
export const listAllElements =
    <StringLiteral extends string | undefined>() =>
        <StringUnion extends StringLiteral[]>(...elements: StringUnion & ([StringLiteral] extends [StringUnion[number]] ? unknown : never)) => elements;
