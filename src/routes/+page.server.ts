...
import {asOrderOption, asPageOption, asPerPageOption, asQOption, asSortOption, searchRepositories} from '$lib/github';

type ViewPageData = InitialViewPageData | ResultViewPageData | ErrorViewPageData;
...
export const load: PageServerLoad = async ({url}): Promise<ViewPageData> => {
    ...
    return Result.combine([asQOption(q), asSortOption(sort), asOrderOption(order), asPerPageOption(per_page), asPageOption(page)])
        .asyncAndThen(([q, sort, order, per_page, page]) => searchRepositories(q, sort, order, per_page, page))
        .match(
            (result) => {
                return {
                    type: 'result',
